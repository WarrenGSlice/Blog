<?php 
	...
	$admin_id = 0;
	$isEditingUser = false;
	$username = $firstname = $lastname = $address = $city = $state = $zip = $country = $email = "";
	...
	* * * * * * * * * * * * * * * * * * * * * * */
	...
	{
		global $conn, $errors, $username, $firstname, $lastname, $address, $city, $state, $zip, $country, $email;
		...
		if (count($errors) == 0) {
			...
			$query = "INSERT INTO users (username, firstname, lastname, address, city, state, zip, country, email, password, created_at, updated_at) 
					  VALUES('$username', '$firstname', '$lastname', '$address', '$city', '$state', '$zip', '$country', '$email', '$password', now(), now())";
	...
	* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * */
	...
	{
		global $conn, $errors, $username, $firstname, $lastname, $address, $city, $state, $zip, $country, $isEditingUser, $admin_id, $email;
		...
		if (count($errors) == 0) {
			...
			$query = "UPDATE users SET username='$username', firstname='$firstname', lastname='$lastname', address='$address', city='$city', state='$state', zip='$zip', country='$country', email='$email', password='$password' WHERE id=$admin_id";
			mysqli_query($conn, $query);
